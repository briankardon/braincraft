...
class Model:
    ...
    def randomize(self):
        ...
        self.Win[0, 0] = amplitude*np.random.uniform(-1, 1) #, size=numNeuronsPerSide)
        # Rightmost distance input to neuron 1
        self.Win[numNeuronsPerSide-1, p-1] = amplitude*np.random.uniform(-1, 1) #, size=numNeuronsPerSide)
        ...
        self.Win[:numSideNeurons, 2*p+2] = amplitude*np.random.uniform(-1, 1, size=numSideNeurons)
        ...
        # self.W[range(numNeuronsPerSide), range(numNeuronsPerSide)] = amplitude*np.random.uniform(-1+weightBias, 1+weightBias, size=[1, numNeuronsPerSide])
        # Interconnect right neurons
        # self.W[range(numNeuronsPerSide, numSideNeurons), range(numNeuronsPerSide, numSideNeurons)] = amplitude*np.random.uniform(-1+weightBias, 1+weightBias, size=[1, numNeuronsPerSide])

        # Chain connect left neurons
        self.W[numNeuronsPerSide-1, list(range(1, numNeuronsPerSide))+[0]] = amplitude*np.random.uniform(-1+weightBias, 1+weightBias) #, size=[1, numNeuronsPerSide])
        # Chain connect right neurons
        self.W[numSideNeurons-1, list(range(numNeuronsPerSide+1, numSideNeurons))+[numNeuronsPerSide]] = amplitude*np.random.uniform(-1+weightBias, 1+weightBias) #, size=[1, numNeuronsPerSide])
...
def mutateNonzeroMatrix(matrix, changeFactors):
    ...
    if numChanges == 0 or len(nonzeros) == 0:
        return
    if numChanges > len(nonzeros):
        numChanges = len(nonzeros)
        changeFactors = changeFactors[:numChanges]
...
def geenius_trainer():
    ...
    try:
        while True:
            ...
            mc = 0.1 + 2*(np.tanh(-2*learning_rate)+1)/2
            print('\nMutation rate:', r)
            print('Mutation size:', mc)
        ...
        print('Skipping rest of training')
        pass
...
if __name__ == "__main__":
    ...
    model = train(geenius_trainer, timeout=10000)
